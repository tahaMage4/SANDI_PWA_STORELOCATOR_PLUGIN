...
class StoreLocatorMap extends PureComponent {
    ...
    // state = {
    //     term: "",  isLoaded: false, error: null
    // };
    ...
    render() {
        const {items, google_api_key, term, isLoaded, error, handleinputfiled} = this.props;
        ...
        return (<>
                ...
                <button onClick={this.getLocation}>
                    <p style={{cursor: 'pointer', color: "darkred"}}>{__('Get Current Location')}</p>
                </button>
                        ...
                        <div className="Search__Items">
                            <form onSubmit={this.onSubmitData}>
                                ...
                                <div style={{marginTop: "15px"}}>
                                    <span>{term.length < 4 ? "At least type 4 letter" : null}</span>
                                </div>
                            </form>
                            {/*left side */}
                                    ...
                                    <div className="shop-list stores__list">
                                        <ol className="shop-list__list hide-lg">
                                            {items && items.map((store) => {
                                                if (term !== "") {
                                                    if (store.name?.toLowerCase().includes(term) || store.description?.toLowerCase().includes(term)) {
